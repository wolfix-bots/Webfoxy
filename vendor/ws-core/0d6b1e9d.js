    ...
    sock.ev.on('messages.upsert', async ({ messages }) => {
        try {
            ...
            for (const message of messages) {
                ...
                if (settings.whitelist.includes(chatJid)) continue;
                ...
                if (shouldRead) {
                    setTimeout(async () => {
                        try {
                            await sock.readMessages([{ remoteJid: chatJid, id: messageId }]);
                        } catch (e) {}
                    }, settings.delay);
                }
            }
...
export default {
    name: 'autoread',
    alias: ['autoview', 'autoseen', 'read', 'foxyread'],
    category: 'utility',
    ownerOnly: true,
...
├─⧭ *Excluded:* ${settings.whitelist.length}
├─⧭ *Forced:* ${settings.blacklist.length}
│
├─⧭ *Commands:*
...
└─⧭🦊`
        ...
        if (subCommand === 'groups') {
            ...
            return await sendMessage(
                `┌─⧭ *✅ GROUPS ONLY* ⧭─┐
│
├─⧭ Only group messages will be read
│
└─⧭🦊`
            );
        }
        ...
        if (subCommand === 'dms') {
            ...
            return await sendMessage(
                `┌─⧭ *✅ DMS ONLY* ⧭─┐
│
├─⧭ Only direct messages will be read
│
└─⧭🦊`
            );
        }
...
│
├─⧭ Use: 0 - 60000 ms
├─⧭ Example: ${PREFIX}autoread delay 3000
│
└─⧭🦊`
                );
            }
